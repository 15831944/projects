...
bool _tagSSockInfo::OndealWithChunckData()
{
	while(0 != s_unCacheHasLen)
	{
		if(!s_bIsChunkedRecvLen)
		{
			...
			if(s_bJumpStart)
			{
				strCache = s_strCacheBuf + 2;
				s_unCacheHasLen -= 2;   //有问题
			}
			else
			{
			...
			if (std::string::npos != pos1)
			{
				...
				if(s_unCacheHasLen > 0)
				{
					int cpyLen = min(s_unThisRemainChunkedLen, s_unCacheHasLen);
					memcpy(s_cDataBuf + s_unHasRecvContentData, s_strCacheBuf + pos1 + 2, cpyLen);
	...
	do 
	{
		...
		else
		{
			...
			else
			{
				...
				{
					memcpy(s_strCacheBuf + s_unCacheHasLen,  recvBuf + pos + 4, recvResult - (pos + 4));
...
bool _tagSSockInfo::RecvContentData(int &errorCode)
{
	...
	do 
	{
		...
		if (0 != s_unContentLen && s_unHasRecvContentData == s_unContentLen)
		{
