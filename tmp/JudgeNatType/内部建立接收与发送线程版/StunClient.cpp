...
bool CStunClient::Start(std::string localIP, WORD localPort, std::string stunServerIP, unsigned short stunServerPort1, unsigned short stunServerPort2)
{
	bool bResult = false;
...
#endif
			m_sink->OnStunFinished(NAT_OPENINTERNET, m_ServerIP, "", 0);
		}
...
#endif
			m_sink->OnStunFinished(NAT_SYMMETRICUDPFIREWALL, m_ServerIP, "", 0);
		}
	}
	else // DIFF_IP
	{
...
#endif
			m_sink->OnStunFinished(NAT_FULLCONE, m_ServerIP, "", 0);
		}
		else
		{
			ResetEvent(m_hEvnetForStep2);
...
#endif
				m_sink->OnStunFinished(NAT_SYMMETRIC, m_ServerIP, "", 0);
			}
			//欠缺未回复的处理？？？？？
			else
			{
...
#endif
					m_sink->OnStunFinished(NAT_PORTRESTRICTEDCONE, m_ServerIP, "", 0);
				}
...
#endif
					m_sink->OnStunFinished(NAT_IPRESTRICTEDCONE, m_ServerIP, "", 0);
				}
			}
		}
	}
...
int CStunClient::Pro_Test1()
{
	...
	//if(WAIT_TIMEOUT == WaitForSingleObject(m_hEvnetForStep1, m_totolWaitSecond))
	//{
	//	return NO_RESPONSE;
	//}
	//else
...
int CStunClient::Pro_Test2()
{
	...
	//if(WAIT_TIMEOUT == WaitForSingleObject(m_hEvnetForStep2, 500))
	//{
	//	m_totolWaitSecond -= 500;
	//	return NO_RESPONSE;
	//}
...
int CStunClient::Pro_Test3()
{
	...
	//if(WAIT_TIMEOUT == WaitForSingleObject(m_hEvnetForStep3, m_totolWaitSecond))
	//{
	//	return NO_RESPONSE;
	//}
	//else
