...
class CA2GZIPT
{
  ...
  void Init(char *lpsz,int len=-1)
  {
	...
	//int err = deflateInit2(&(m_zstream), t_nLevel,Z_DEFLATED, -MAX_WBITS, 8, t_nStrategy);
	...
	//GZIP header[10]={0x1f,0x8b,Z_DEFLATED, 0 /*flags*/, 0,0,0,0 /*time*/, 0 /*xflags*/, 0x0b};
    write(header,10);
...
class CGZIP2AT
{
 ...
 int write(char* buf,int count)
 {
	 ...
	 if(Length+count>m_CurrentBufferSize)
	 {
	   ...
	   //add by pgp
	   //发现在memset(psz,0,m_CurrentBufferSize+1)时崩溃
	   //崩溃原因是write了无效内存.在这判断下
	   if(psz==0)
		   return 0;
	   m_CurrentBufferSize=nTimes*t_nBufferLength;
	   memset(psz,0,m_CurrentBufferSize+1);
